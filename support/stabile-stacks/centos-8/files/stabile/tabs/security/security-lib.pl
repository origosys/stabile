...
sub get_limit {
    my $limit;
    my $conf = "/etc/httpd/conf.d/webmin-ssl";
...
sub set_limit {
    ...
    # Handle name change in Xenial
    $conf .= '.conf' if (-e "$conf.conf");
    $cmd = qq|perl -pi -e 's/allow from (.*) \#stabile/allow from $validlimit #stabile/;' $conf|;
    $message .= `$cmd`;
