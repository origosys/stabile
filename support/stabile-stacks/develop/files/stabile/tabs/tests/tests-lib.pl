...
<span class="dropdown">
    ...
    <ul class="dropdown-menu dropdown-inverse">
        <li>
            <a href="#" onclick="\$('#myChart').empty(); \$('#wait_test').html(waittxt); return false;">Show tests</a>
        </li>
        <li>
...
# Generate and return javascript the UI for this tab needs
    ...
    var waittxt = '<div class="well" style="font-size:12px; margin-top:10px; max-width:800px;">\\
...
Disk: dd bs=1M count=64 if=/dev/zero of=/tmp/ddtest conv=fdatasync oflag=direct<br>\\
NFS: dd bs=1M count=64 if=/dev/zero of=/mnt/fuel/POOL/ddtest conv=fdatasync oflag=direct<br>\\
...
Network gateway: netperf -f M -H $gw -l 10<br>\\
    ...
    function getTestResults(servs, test) {
        ...
        .done(function( data ) {
            if (data) {
                \$("#run_test" ).prop( "disabled", false );
                \$("#wait_test").html('<span class="label" style="color:gray;">' + data.name + '</span>');
                var options = {
                    barShowStroke: true,
                    responsive: true,
...
#        "dd", "dd bs=128M count=1 if=/dev/zero of=/tmp/ddtest conv=fdatasync oflag=direct",
        "dd", "dd bs=1M count=64 if=/dev/zero of=/tmp/ddtest conv=fdatasync oflag=direct",
    ...
    my $scommand;
    push @servers, $servers[0] if ($test eq 'network' && (scalar @servers) ==1); # We need two servers to test network...
            ...
            if ($test eq 'iops' || $test eq 'iopsdirect' || $test eq 'io' || $test eq 'iodirect') {
                        ...
                        foreach my $number (@numbers) {
                            if ($number) {
                                ...
                                unless ($testkey eq 'KB' || $testkey eq 'reclen') {
                                    ...
                                    if ($test eq 'io' || $test eq 'iodirect') {
                                        my @b1 = @{$tests{$testkey}}; push @b1, int($number/1024);
                                        $tests{$testkey} = \@b1;
                                    } else {
                ...
                for (split /^/, $result)  {
                    ...
                    if ($test eq 'dd' || $test eq 'ddnfs') {
                        ...
                        my $testkey = 'transfer speed';
                        unless ($tests{$testkey}){my @a1; $tests{$testkey} = \@a1;}
                        if ($_ =~ /transferred \((\S+) MiB\/sec\)/) {
                            my @b1 = @{$tests{$testkey}}; push @b1, $1;
                            $tests{$testkey} = \@b1;
                        }
