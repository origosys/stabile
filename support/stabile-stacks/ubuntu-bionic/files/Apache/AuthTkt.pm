...
sub parse_ticket
{
    ...
    # Deconstruct
    my ($digest,$ts,$uid,$extra);
    if ($self->digest_type eq 'MD5') {
        ($digest,$ts,$uid,$extra) = ($raw =~ m/^(.{32})(.{8})(.+?)!(.*)$/); # MD5
    } elsif ($self->digest_type eq 'SHA256') {
        ($digest,$ts,$uid,$extra) = ($raw =~ m/^(.{64})(.{8})(.+?)!(.*)$/); # SHA256
    } else {
        ($digest,$ts,$uid,$extra) = ($raw =~ m/^(.{128})(.{8})(.+?)!(.*)$/); # SHA512
    }
    $parts->{digest} = $digest;
    $parts->{ts}  = hex($ts);
    $parts->{uid} = $uid;
...
=head1 SYNOPSIS
    ...
    $at = Apache::AuthTkt->new(
        conf => '/etc/apache2/conf.d/auth_tkt_cgi.conf',
    );
    # OR:
    $at = Apache::AuthTkt->new(
