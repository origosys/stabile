...
## This endpoint is used by stabile-ubuntu.pl, which is run as a startup script
    ...
    if ($mip) {
        ...
        require "/usr/share/webmin/useradmin/user-lib.pl";
        require "/usr/share/webmin/useradmin/md5-lib.pl";
        # my $crypt_pass = &encrypt_password($pass, undef, 'force_system_detection');
        my $crypt_pass = &encrypt_md5($pass);
        `perl -pi -e 's/admin:.*\\n?//s' /etc/webmin/miniserv.users`;
        open(FH, '>>', "/etc/webmin/miniserv.users") or die $!;
        print FH "admin:$crypt_pass:0";
        close(FH);
        ...
        # my $status = system(qq|/usr/share/webmin/bin/webmin passwd --config "/etc/webmin" --user "admin" --pass "$pass"|);
